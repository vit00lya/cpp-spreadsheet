...
void TestErrorValue() {
    ...
    auto result = sheet->GetCell("E4"_pos)->GetValue();
    ASSERT_EQUAL(result,
                    CellInterface::Value(FormulaError::Category::Value));
}
...
void TestPrint3() {
   auto sheet = CreateSheet();
    sheet->SetCell("D5"_pos, "D5");
    sheet->SetCell("C4"_pos, "C4");
    auto size = sheet->GetPrintableSize();
    ASSERT_EQUAL(size.cols, 4);
    ASSERT_EQUAL(size.rows, 5);
    std::ostringstream texts;
    std::ostringstream values;
   ...
   ASSERT_EQUAL(values.str(), "\t\t\t\n\t\t\t\n\t\t\t\n\t\tC4\t\n\t\t\tD5\n");
    ...
    sheet->ClearCell("D5"_pos);
    auto size2 = sheet->GetPrintableSize();
    ASSERT_EQUAL(size2.cols, 3);
    ASSERT_EQUAL(size2.rows, 4);

    std::ostringstream texts2;
    std::ostringstream values2;
    sheet->PrintTexts(texts2);
    sheet->PrintValues(values2);
    ASSERT_EQUAL(texts2.str(), "\t\t\n\t\t\n\t\t\n\t\tC4\n");
